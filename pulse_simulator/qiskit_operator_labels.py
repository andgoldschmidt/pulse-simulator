...
def to_label(index_char_dict, length):
    """Make an operator string where the character is placed at the index for
    each entry of the dictionary, or an "I" is added.

    Arguments:
        index_char_dict -- a dictionary of {Integer: Character} entries.
        length -- the number of entries in the string
    ...
    for i in range(length):
        if i in index_char_dict.keys():
            label += index_char_dict[i]
        else:
            label += "I"
    return label

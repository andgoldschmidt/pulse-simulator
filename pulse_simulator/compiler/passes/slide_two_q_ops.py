...
class SlideTwoQubitOps(TransformationPass):
    def __init__(self):
        """Sends two-qubit operations to as soon as possible without while leaving one-qubit layers alone."""
        super().__init__()
    ...
    def run(self, dag: DAGCircuit) -> DAGCircuit:
        ...
        # ensure we start with a two-qubit moment
        current_index = 0
        if moments[0][1] == "one":
            current_index += 1

        last_two_q_moment = None
        used_qargs = set()
        for i in range(current_index, len(moments), 2):
            # grab one- and two-qubit moments
            if i - 1 >= 0:
                one_q_moment = moments[i - 1][0]
            else:
                one_q_moment = [DAGCircuit()]
            two_q_moment = moments[i][0]

            for layer in one_q_moment:
            ...
            if i > 1:
                removed = []
                for gate in two_q_moment[0].op_nodes():
                    if all([q not in used_qargs for q in gate.qargs]):
                        last_two_q_moment.apply_operation_back(
                            gate.op, qargs=gate.qargs, cargs=gate.cargs
                        )
                        removed.append(gate)
                for gate in removed:
                    two_q_moment[0].remove_op_node(gate)

            # keep track of used wires
            used_qargs = set()
            if len(two_q_moment) == 0:
                continue
            for gate in two_q_moment[-1].op_nodes():
                for q in gate.qargs:
                    used_qargs.add(q)
            last_two_q_moment = two_q_moment[-1]
